...
glm::vec3 new_material_ambient = { 0.5, 0.05, 0.005 }, new_material_diffuse = { 0.5, 0.5, 0.5 }, new_material_specular = { 0.5, 0.5, 0.5}; GLfloat new_material_shininess = 5.5f;
...
void init_waving_plane() {
	...
	obj4.shading = ShadingType::WAVE;  //GOURAUD;WAVE;
	obj4.name = "Waves";
...
void init_mesh() {
	...
	obj4.shading = ShadingType::TOON; // GOURAUD; // PHONG;
	obj4.name = "Bunny";
...
void initShader()
{
	...
	light_uniforms.resize(7); // allocate space for uniforms of PHONG, BLINN and GOURAND + TOON
	...
	float r = materials[objects[selected_obj].material].ambient.r;
	float g = materials[objects[selected_obj].material].ambient.g;
	float b = materials[objects[selected_obj].material].ambient.b;
	glUniform4fv(glGetUniformLocation(shaders_IDs[TOON], "Color"), 1, value_ptr(glm::vec4(materials[objects[selected_obj].material].ambient, 1.0)));
	glUniform4fv(glGetUniformLocation(shaders_IDs[TOON], "Color"), 1, value_ptr(glm::vec4(1, 1, 1, 1.0)));
...
void init() {
	...
	materials[MaterialType::NEW_MATERIAL].name = "New Material";
	materials[MaterialType::NEW_MATERIAL].ambient = new_material_ambient;
	materials[MaterialType::NEW_MATERIAL].diffuse = new_material_diffuse;
	materials[MaterialType::NEW_MATERIAL].specular = new_material_specular;
	materials[MaterialType::NEW_MATERIAL].shininess = new_material_shininess;
...
void buildOpenGLMenu()
{
	...
	glutAddMenuEntry(materials[MaterialType::NEW_MATERIAL].name.c_str(), MaterialType::NEW_MATERIAL);
...
void moveCameraLeft()
{
	...
	glm::vec3 upDirection = glm::normalize(slide_vector) * CAMERA_TRASLATION_SPEED;
...
void modifyModelMatrix(glm::vec3 translation_vector, glm::vec3 rotation_vector, GLfloat angle, GLfloat scale_factor)
{
	...
	glm::mat4 scale = glm::scale(glm::mat4(1.0), glm::vec3 (scale_factor, scale_factor,scale_factor));
